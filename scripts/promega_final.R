...
final <- data.frame(matrix(ncol = 0, nrow = length(unique(data$Content))))
...
final$`[75bp]:[300bp] (Avg SQ [75]/[300])` <- drop$Quasar.670.SQ.Mean/drop$Cal.Gold.540.SQ.Mean
final$`[75bp]:[150bp] (Avg SQ [75]/[150])` <- drop$Quasar.670.SQ.Mean/drop$FAM.SQ.Mean

#output to csv; you can change "promega" to whatever file name you want. This will save to your working directory.
write.csv(final, "promega_final.csv", row.names = FALSE)
